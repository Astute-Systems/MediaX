...
/// encoding-name=(string)H264" ! rtph264depay ! h264parse ! queue ! omxh264dec ! caps="video/x-raw, format=RGB" !
...
RtpH264GstOmxDepayloader::RtpH264GstOmxDepayloader() {
...
RtpH264GstOmxDepayloader::~RtpH264GstOmxDepayloader() = default;

RtpH264GstOmxDepayloader &RtpH264GstOmxDepayloader::operator=(const RtpH264GstOmxDepayloader &other [[maybe_unused]]) {
  return *this;
}

void RtpH264GstOmxDepayloader::SetStreamInfo(const ::mediax::rtp::StreamInformation &stream_information) {
...
GstFlowReturn RtpH264GstOmxDepayloader::NewFrameCallback(GstAppSink *appsink, gpointer user_data) {
  gint width = 0;
  gint height = 0;
  auto depayloader = static_cast<RtpH264GstOmxDepayloader *>(user_data);
...
bool RtpH264GstOmxDepayloader::Open() {
  ...
  // Decode frame using omx
  GstElement *omxh264dec = gst_element_factory_make("openh264dec", "rtp-h264-openh264dec");
  ...
  GstAppSinkCallbacks callbacks = {.new_sample = RtpH264GstOmxDepayloader::NewFrameCallback};
  ...
  if (rgb_) {
    ...
    gst_bin_add_many(GST_BIN(pipeline_), udpsrc, capsfilter, rtph264depay, h264parse, queue, omxh264dec, videoconvert,
                     capsfilter2, appsink, nullptr);

    // Link the elements
    gst_element_link_many(udpsrc, capsfilter, rtph264depay, h264parse, queue, omxh264dec, videoconvert, capsfilter2,
                          appsink, nullptr);
  } else {
    // Add all elements to the pipeline
    gst_bin_add_many(GST_BIN(pipeline_), udpsrc, capsfilter, rtph264depay, h264parse, queue, omxh264dec, appsink,
                     nullptr);

    // Link the elements
    gst_element_link_many(udpsrc, capsfilter, rtph264depay, h264parse, queue, omxh264dec, appsink, nullptr);
  }

  return true;
}

void RtpH264GstOmxDepayloader::Start() {
  // Return if not open
...
void RtpH264GstOmxDepayloader::Stop() {
  if (GetState() != ::mediax::rtp::StreamState::kStarted) {
    return;
  }
  // Call the base class
...
void RtpH264GstOmxDepayloader::Close() {
...
bool RtpH264GstOmxDepayloader::Receive(mediax::rtp::RtpFrameData *data, int32_t timeout) {
...
void RtpH264GstOmxDepayloader::Callback(::mediax::rtp::RtpFrameData frame) const {
  if (GetState() == ::mediax::rtp::StreamState::kStarted) {
    GetCallback()(static_cast<const RtpDepayloader &>(*this), frame);
  }
}

void RtpH264GstOmxDepayloader::NewFrame() {
  new_rx_frame_ = true;
