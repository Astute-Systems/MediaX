...
RtpJpegGstVaapiPayloader::RtpJpegGstVaapiPayloader() = default;

RtpJpegGstVaapiPayloader::~RtpJpegGstVaapiPayloader() = default;

void RtpJpegGstVaapiPayloader::SetStreamInfo(const ::mediax::rtp::StreamInformation &stream_information) {
...
int RtpJpegGstVaapiPayloader::Transmit(unsigned char *new_buffer, bool timeout) {
  if (!started_) {
...
bool RtpJpegGstVaapiPayloader::Open() {
  ...
  GstElement *h265enc = gst_element_factory_make("vaapijpegenc", "rtp-jpeg-enc");
...
void RtpJpegGstVaapiPayloader::Close() {
  // Stop the pipeline
  Stop();
...
void RtpJpegGstVaapiPayloader::Start() {
  started_ = true;
  // Start the pipeline
  gst_element_set_state(pipeline_, GST_STATE_PLAYING);
}

void RtpJpegGstVaapiPayloader::Stop() {
  started_ = false;
  // Stop the pipeline
