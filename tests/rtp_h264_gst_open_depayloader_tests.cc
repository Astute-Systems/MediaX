...
TEST(RtpH264OpenDepayloaderTest, Timeout) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
  ...
  rtp = std::make_shared<mediax::rtp::h264::gst::open::RtpH264GstOpenDepayloader>();
...
TEST(RtpH264OpenDepayloaderTest, GetSet) {
...
TEST(RtpH264OpenDepayloaderTest, UnicastOk) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
  ...
  WritePngFile(rgb_test.data(), rtp.GetWidth(), rtp.GetHeight(), "openH264_Image.png");
}

TEST(RtpH264OpenDepayloaderTest, UnicastOkSetStreamInfo) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
  ...
  WritePngFile(rgb_test.data(), rtp.GetWidth(), rtp.GetHeight(), "openH264_Image1.png");
}

TEST(RtpH264OpenDepayloaderTest, UnicastOkSetStreamInfoPtr) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
  ...
  std::array<uint8_t, 1280 * 720 * 4> rgb_test;
  mediax::video::ColourSpaceCpu colourspace;
  auto rtp = std::make_shared<mediax::rtp::h264::gst::open::RtpH264GstOpenDepayloader>();
  ...
  WritePngFile(rgb_test.data(), rtp->GetWidth(), rtp->GetHeight(), "openH264_Image2.png");
}

TEST(RtpH264OpenDepayloaderTest, kColourspaceRgba) {
  mediax::rtp::h264::gst::open::RtpH264GstOpenDepayloader rtp;
...
TEST(RtpH264OpenDepayloaderTest, FailToOpen) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
...
TEST(RtpH264OpenDepayloaderTest, TestNoStreamSet) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
...
void Stream(mediax::rtp::h264::gst::open::RtpH264GstOpenDepayloader* rtp, std::string name, std::string ip,
            uint16_t port) {
  // Set the stream details using set stream info
...
TEST(RtpH264OpenDepayloaderTest, StartSwitchManyPayloaders) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
  ...
  std::array<uint8_t, 1280 * 720 * 3> rgb_test;
  mediax::rtp::h264::gst::open::RtpH264GstOpenDepayloader rtp[5];
...
TEST(RtpH264OpenDepayloaderTest, TransmitAFrame) {
#if !GST_SUPPORTED
  GTEST_SKIP();
#endif
  ...
  // Transmit a frame
  mediax::rtp::h264::gst::open::RtpH264GstOpenPayloader rtp_pay;
